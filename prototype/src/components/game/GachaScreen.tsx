...
export default function GachaScreen({ save, onPull, onBack }: GachaScreenProps) {
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(to bottom, #0F0A1A, #2D1B4E 50%, #1A1A2E)`,
      }}
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 bg-[#0D0D1A]/80">
        <button
          onClick={onBack}
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <div className="relative mb-8">
          ...
          {isAnimating && (
            <div
              className="absolute -inset-16 rounded-full animate-ping opacity-30"
              style={{
        ...
        {/* Rate info */}
        <div
          className="px-4 py-2 rounded-lg border mb-6 text-center"
          style={{
            borderColor: '#444',
        ...
        >
          ...
          <p className="text-[10px] mt-1" style={{ color: COLORS.primary }}>
            10연차 시 ★3 이상 1명 보장
          </p>
        </div>
        ...
        <div className="flex gap-4">
          <button
            ...
            className="px-6 py-3 rounded-lg font-bold border-2 transition-all hover:scale-105 active:scale-95 disabled:opacity-40 disabled:hover:scale-100"
            style={{
          ...
          >
            ...
            <p className="text-xs" style={{ color: '#9C27B0' }}>
          ...
          >
            ...
            <p className="text-xs opacity-80">
              💎 {GACHA_CONFIG.tenPullCost} (10% 할인)
            </p>
          </button>
        </div>
      </div>
      ...
      {results && (
        ...
        >
          ...
          <div className="flex flex-wrap gap-3 justify-center px-4 max-w-lg">
            {results.map((result, i) => {
              const starColor = COLORS.starColors[result.stars] || '#9E9E9E';
              return (
                <div
                  key={i}
                  className={`w-24 p-2 rounded-lg border-2 flex flex-col items-center ${
                    result.isNew ? 'animate-bounce' : ''
                  }`}
                  style={{
                ...
                >
                  <div className="flex gap-0.5">
                    {Array.from({ length: result.stars }).map((_, j) => (
                      <span key={j} className="text-[8px]" style={{ color: starColor }}>★</span>
                    ))}
                  </div>
                  <div className="my-1">
                    <CharacterAvatar definitionId={result.heroId} emoji={result.emoji} size="lg" />
                  </div>
                  <p className="text-[10px] font-bold" style={{ color: COLORS.textPrimary }}>
                    {result.nameKo}
                  </p>
                  {result.isNew ? (
                    <span className="text-[9px] px-1 rounded" style={{ backgroundColor: COLORS.accent, color: '#fff' }}>
                      NEW!
                    </span>
                  ) : (
          ...
          <button
            onClick={handleClose}
            className="mt-6 px-6 py-2 rounded-lg border font-bold"
            style={{ borderColor: COLORS.primary, color: COLORS.primary }}
          >
            확인
          </button>
