...
⚠️ **API Key 보안 규칙 (필수 준수)**:
- **절대로 API Key를 문서, 코드, 커밋 메시지, 로그 등 어떤 파일에도 기록하지 않는다**
- `docs/05-asset-list.md` 등 에셋 목록 문서에 키 값을 포함하지 않는다 (상태만 "확인됨/미제공"으로 기록)
- API Key는 오직 `curl` 명령어 실행 시 환경변수(`$GEMINI_API_KEY`)로만 참조한다
- Git에 커밋되는 모든 파일에 API Key, 토큰, 시크릿 등 민감 정보가 포함되지 않도록 한다
- 실수로 노출된 경우 즉시 git 히스토리에서 제거하고, 사용자에게 키 폐기/재발급을 안내한다
